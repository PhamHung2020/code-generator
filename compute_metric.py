...
def main():
    ...
    taco = load_from_disk("dataset/train").select([1, 3])
    print(taco[0])

    # generation_file = 'generations.json'
    ...
    # import uuid
    # for task_id, res in results.items():
    #     solutions = generations[task_id]
    #     for score, solution in zip(res, solutions):
    #         print(task_id, score)
    #         score_np = np.array(score)
    #         if np.any(score_np > 0):
    #             output = {'task_id': task_id, 'solution_id': str(uuid.uuid4()), 'solution': solution, 'result': score}
    #             outputs.append(output)

    # import csv
    # with open('result.csv', 'w', newline='') as outfile:
    #     writer = csv.DictWriter(outfile, outputs[0].keys())
    #     writer.writeheader()
